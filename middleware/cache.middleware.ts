...
export default async function cacheControl(fastify: any, opts: any) {
  fastify.addHook('onRequest', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (method === 'GET') {
      ...
      if (
        url.match(/\.(js|css|png|jpg|jpeg|gif|ico|svg|woff|woff2|ttf|eot|webp)$/) ||
        url.startsWith('/uploads/') ||
        url.startsWith('/static/') ||
        url.startsWith('/assets/')
      ) {
        reply.header('Cache-Control', 'public, max-age=2592000, immutable'); // 30 days
        reply.header('Cloudflare-CDN-Cache-Control', 'public, max-age=2592000');
      ...
      if (
        ...
        reply.header('Cloudflare-CDN-Cache-Control', 'public, max-age=31536000');
      }
