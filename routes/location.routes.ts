...
export default async function (fastify: FastifyInstance) {
        ...
        if (!lat || !lng) {
          return reply.code(400).send({
            ...
            error: {
              ...
              message: 'Latitude (lat) and longitude (lng) are required parameters'
            }
          });
        }
   ...
   */
  fastify.get<{ Querystring: SearchQuery }>('/api/locations/search', async (req, reply) => {
    try {
      ...
      const results = features.map(feature => ({
        ...
        address: {
          ...
          municipality: feature.context?.find(ctx => ctx.id.includes('locality'))?.text,
          country: feature.context?.find(ctx => ctx.id.includes('country'))?.text,
          country_code: feature.context?.find(ctx => ctx.id.includes('country'))?.short_code
        },
        namedetails: {
          name: feature.text,
   ...
   */
  fastify.get<{ Querystring: ReverseQuery }>('/api/locations/reverse', async (req, reply) => {
    try {
