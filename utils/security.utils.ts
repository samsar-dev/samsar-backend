...
type Sanitizable = SanitizablePrimitive | Sanitizable[] | { [key: string]: SanitizablePrimitive | Sanitizable[] | { [key: string]: unknown } };
...
export const requestSanitizer = async (
  ...
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : 'Unknown error during request sanitization';
