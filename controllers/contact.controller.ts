...
const verifyAdminToken = (request: FastifyRequest, reply: FastifyReply): boolean => {
  try {
...
export const getContactSubmissions = async (
  ...
  try {
    ...
    return reply.status(500).send({ error: 'Failed to fetch contact submissions' });
  }
};
...
export const markAsRead = async (
  ...
  try {
    ...
    return reply.status(500).send({ error: 'Failed to mark submission as read' });
  }
};
