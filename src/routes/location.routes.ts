...
// TODO: Replace these with actual middleware imports once created
const authenticate = async (req: AuthRequest, _reply: FastifyReply, next: (err?: Error) => void) => {
...
const processImagesMiddleware = async (req: MultipartAuthRequest, _reply: FastifyReply, next: (err?: Error) => void) => {
  // This is a temporary implementation
  console.log('Image processing middleware called');
  req.processedImages = [];
  return next();
};
...
interface MultipartAuthRequest extends AuthRequest {
...
const formatListingResponse = (listing: any): ListingWithRelations | null => {
  ...
  return {
    ...
    category: listing.mainCategory + (listing.subCategory ? `/${listing.subCategory}` : ''),
